...
""")
    ...
    respuesta = st.radio(
        ...
        ["Entrenamiento", "Evaluación", "Serialización", "Predicción"],
        index=None,
        key="lifecycle_reflection",
    )

    if st.button("Comprobar respuesta", key="check_lifecycle_reflection"):
        if respuesta is None:
            st.warning("Selecciona una opción antes de comprobarla.")
        elif respuesta == "Serialización":
            st.success("🎉 Correcto.")
        else:
            st.error("Observa nuevamente el flujo del ciclo de vida.")

    st.button(
        "Reintentar",
        key="reset_lifecycle_reflection",
        on_click=lambda: st.session_state.pop("lifecycle_reflection", None),
    )
