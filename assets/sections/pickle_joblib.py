...
""")
    ...
    decision = st.radio(
        ...
        ["Pickle", "Joblib"],
        index=None,
        key="pickle_joblib_decision",
    )

    if st.button("Comprobar respuesta", key="check_pickle_joblib_decision"):
        if decision is None:
            st.warning("Selecciona una opción antes de comprobarla.")
        elif decision == "Joblib":
            st.success("🎉 Correcto.")
        else:
    ...
    st.button(
        "Reintentar",
        key="reset_pickle_joblib_decision",
        on_click=lambda: st.session_state.pop("pickle_joblib_decision", None),
    )
...
Ahora estás listo para poner en práctica lo aprendido en el reto Colab.
""")
